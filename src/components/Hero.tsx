import { CourseInfo, Lecturer, TeamMember, GroupActivities } from "@/utils/markdownParser";
import { GraduationCap, Users, Mail, Github, Calendar } from "lucide-react";
...
export const Hero = ({ courseInfo, lecturer, teamMembers, groupActivities }: HeroProps) => {
  return (
    <div className="relative overflow-hidden">
      ...
      <div className="relative container mx-auto px-6 py-16">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {/* Group Activities - ngay dưới ảnh thành viên */}
          {groupActivities && (
            <div className="mt-6 text-center">
              <a
                href={groupActivities.url}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/30 text-primary hover:bg-primary/20 transition-colors"
              >
                <Calendar className="w-4 h-4" />
                <span className="text-sm font-medium">Hoạt động nhóm: {groupActivities.title}</span>
              </a>
            </div>
          )}
        </div>
